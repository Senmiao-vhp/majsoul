...
class Wall:
    ...
    def initialize(self) -> None:
        ...
        for suit in [TileSuit.MAN, TileSuit.PIN, TileSuit.SOU]:
            for number in range(1, 10):
                for i in range(4):
                    # 5万/5筒/5索中各有一张赤宝牌
                    is_red = number == 5 and i == 0
                    self.tiles.append(Tile(suit, number, is_red))
    ...
    def setup_dead_wall(self) -> None:
        ...
        # 设置宝牌指示牌
        self.dora_indicators = [self.dead_wall_tiles[8]]  # 第9张作为宝牌指示牌
        self.uradora_indicators = [self.dead_wall_tiles[9]]  # 第10张作为里宝牌指示牌
    ...
    def add_dora_indicator(self) -> None:
        """翻开新的宝牌指示牌"""
        if len(self.dora_indicators) >= 5:  # 最多5张宝牌指示牌
            return
        ...
        if next_index >= 0 and next_index < len(self.dead_wall_tiles):
            ...
            self.dora_indicators.append(next_indicator)
    
    def reveal_uradora(self) -> None:
        """翻开里宝牌指示牌"""
        if len(self.uradora_indicators) >= 5:  # 最多5个里宝牌指示牌
            return
        ...
        if next_index >= 0 and next_index < len(self.dead_wall_tiles):
            next_indicator = self.dead_wall_tiles[next_index]
            self.uradora_indicators.append(next_indicator)
    
    def handle_kan_dora(self) -> None:
        """处理杠宝牌"""
        if len(self.dora_indicators) < 5:  # 最多5个宝牌指示牌
